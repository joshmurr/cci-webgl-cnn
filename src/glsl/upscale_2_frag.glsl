...
vec3 get(vec2 _st, vec2 _filter_offset, vec2 _offset){
  ...
  vec3 filter_lookup = texture(u_filter, _filter_offset + (((_offset*2.0)+vec2(1.5)) * u_filter_texel_size)).rgb * 2.0 - 1.0;
...
void main(){
  ...
  outColor = vec4(sum_filters / 16.0, 1.0) + (texture(u_bias, filter_offset + (1.0 / u_num_filters)) * 2.0 - 1.0);
